...
if __name__ == '__main__':
    ...
    with open(args.preds, 'r', encoding='utf-8') as preds_file:
        rewritten = preds_file.readlines()
        rewritten = [sentence.strip() for sentence in rewritten]

    print('Models loading...')
    style_model, style_tokenizer = load_model('s-nlp/russian_toxicity_classifier', use_cuda=args.use_cuda)
    meaning_model, meaning_tokenizer = load_model('cointegrated/LaBSE-en-ru', use_cuda=args.use_cuda,
                                                  model_class=AutoModel)
    fluency_model, fluency_tolenizer = load_model('s-nlp/rubert-base-corruption-detector',
